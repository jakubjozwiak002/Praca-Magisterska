...
def run_method_tophat(source_path, source_type, get_params_callback, root):
    ...
    if is_video:
        ...
        if not ret:
            print("[Top-Hat] Błąd odczytu strumienia wideo.")
            return [], [] 
        is_playing = False
    else:
        current_frame = cv2.imread(source_path)
        if current_frame is None:
            print("[Top-Hat] Błąd odczytu obrazu.")
            return [], [] 
        is_playing = False

    cv2.namedWindow("Wynik Koncowy (Top-Hat)", cv2.WINDOW_NORMAL)
    ...
    while True:
        ...
        valid_detections_info = [] # Tymczasowa lista detekcji do analizy fotometrycznej
        ...
        # Parametry Detekcji (Top-Hat)
        tophat_k = params.get('tophat_k', 9)
        thresh_val = params.get('thresh_val', 125)
        min_a = params.get('min_a', 5)
        max_a = params.get('max_a', 1500)
        max_aspect_ratio_raw = params.get('max_aspect', 30)
        
        # Flaga fotometrii przekazywana z GUI
        enable_photometry = params.get('enable_photometry', False)

        if tophat_k < 1: tophat_k = 1
        if tophat_k % 2 == 0: tophat_k += 1
        if max_a <= min_a: max_a = min_a + 1
        max_aspect_ratio = max_aspect_ratio_raw / 10.0
        if max_aspect_ratio < 1.0: max_aspect_ratio = 1.0
        ...
        tophat_roi = cv2.bitwise_and(tophat, tophat, mask=mask_roi)
            ...
            if min_a <= area <= max_a:
                ...
                if aspect_ratio <= max_aspect_ratio:
                    ...
                    if M["m00"] != 0:
                        ...
                        x2_pad = min(w, x_rect + bw + PADDING)
                        y2_pad = min(h, y_rect + bh + PADDING)
        ...
        for det in valid_detections_info:
            ...
            x2_csv = det['x_rect'] + det['bw']
            y2_csv = det['y_rect'] + det['bh']
        ...
        cv2.imshow("Wynik Koncowy (Top-Hat)", final_display)
