...
# ==============================================================================
# Wersja przenośna dla Linux/Raspberry Pi.
# Domyślnie zakłada strukturę:
#
# 00_TESTY/
# ├── master_benchmark_rpi5.py
# ├── test/
# │   ├── images/
# │   │   └── dataset_config.json
# │   └── labels/
...
#     └── 26/yolov26.pt
#
# Ścieżki można nadpisać zmiennymi środowiskowymi bez edycji kodu, np.:
# export BENCHMARK_DATASET_DIR="/home/kubat/Downloads/00_TESTY/test"
# export BENCHMARK_MODELS_DIR="/home/kubat/Downloads/00_TESTY/modele"
# export BENCHMARK_RESULTS_CSV="/home/kubat/Downloads/00_TESTY/benchmark_results_rpi5.csv"
...
# Plik JSON zapisany w folderze images.
CONFIG_FILE = Path(os.environ.get("BENCHMARK_CONFIG_FILE", IMAGES_DIR / "dataset_config.json")).expanduser().resolve()

RESULTS_CSV = Path(os.environ.get("BENCHMARK_RESULTS_CSV", BASE_DIR / "benchmark_results_rpi5.csv")).expanduser().resolve()

# Nazwa platformy zapisywana w pliku wynikowym.
PLATFORM_NAME = os.environ.get("BENCHMARK_PLATFORM", "RPI5_CPU")

# Wymuszenie inferencji CPU dla Raspberry Pi 5 bez Hailo.
YOLO_DEVICE = os.environ.get("BENCHMARK_YOLO_DEVICE", "cpu")

# Opcjonalne ograniczenie liczby wątków. 0 oznacza brak wymuszenia.
OPENCV_THREADS = int(os.environ.get("BENCHMARK_OPENCV_THREADS", "0"))
TORCH_THREADS = int(os.environ.get("BENCHMARK_TORCH_THREADS", "0"))

if OPENCV_THREADS > 0:
    cv2.setNumThreads(OPENCV_THREADS)

if torch is not None and TORCH_THREADS > 0:
    try:
        torch.set_num_threads(TORCH_THREADS)
        torch.set_num_interop_threads(max(1, min(TORCH_THREADS, 4)))
    except Exception as exc:
        print(f"Ostrzeżenie: nie udało się ustawić liczby wątków PyTorch: {exc}")
...
PRESETS = ['Dzien', 'Noc']
MODELS_DIR = Path(os.environ.get("BENCHMARK_MODELS_DIR", BASE_DIR / "modele")).expanduser().resolve()

YOLO_MODELS = [
    MODELS_DIR / "8" / "yolov8.pt",
    MODELS_DIR / "11" / "yolov11.pt",
    MODELS_DIR / "12" / "yolov12.pt",
    MODELS_DIR / "26" / "yolov26.pt",
]
...
class MasterBenchmark:
    def validate_paths(self):
        """Sprawdza ścieżki przed rozpoczęciem benchmarku, aby uniknąć błędów w trakcie testu."""
        missing = []
        ...
        if missing:
            ...
            print("  00_TESTY/")
            print("  ├── master_benchmark_rpi5.py")
            print("  ├── test/images/dataset_config.json")
            print("  ├── test/labels/")
            print("  └── modele/8, modele/11, modele/12, modele/26")

            print("\nMożesz też nadpisać ścieżki zmiennymi środowiskowymi, np.:")
            print('  export BENCHMARK_DATASET_DIR="/home/kubat/Downloads/00_TESTY/test"')
            print('  export BENCHMARK_MODELS_DIR="/home/kubat/Downloads/00_TESTY/modele"')
            raise FileNotFoundError("Brak wymaganych plików/katalogów benchmarku.")
    ...
    def detect_yolo_headless(self, model, frame, mask, photometry_on):
        ...
        results = model.predict(source=frame, conf=0.4, imgsz=imgsz, device=YOLO_DEVICE, verbose=False)
    ...
    def run(self):
        ...
        for algo, res, preset, photo_on in combinations:
            ...
            for yolo_model_path in models_to_test:
                model_name = Path(yolo_model_path).name if yolo_model_path else "N/A"
                ...
                yolo_net = YOLO(str(yolo_model_path)) if algo == 'YOLO' else None
                ...
                if algo == 'YOLO':
                    ...
                    for _ in range(3):
                        _ = yolo_net.predict(source=dummy, conf=0.4, imgsz=max(res), device=YOLO_DEVICE, verbose=False)
